<%@ Page Title="" Language="VB" MasterPageFile="~/ContentMasterPage2.master" AutoEventWireup="false" CodeFile="frmScrollingTextScheule2.aspx.vb" Inherits="frmScrollingTextScheule2" %>
                       ...
                       <%-- <div class="span-2"></div>
                        ...
                        <div class="row-fluid">
                         ...
                         <div class="row-fluid">
                            ...
                            <div class="span9" style="margin-right:0px">
                                <label class="control-label" id="lblAutoStopDurationMS">Minute</label>
                                 <span class="help-inline color red" id="lblvalidDuration" style="display: none">This field is required</span>
                            </div>
...
<script type="text/javascript">
    ...
    function onSaveTime(id) {
        ...
        var text_speed_id = $('#cbTextSpeed').val();
        var text_speed_name = $('#cbTextSpeed>option:selected').text();
        ...
        if (id == 0) {
            aaData.push({
                ...
                "speed_level_id": text_speed_id,
                "speed_level_name": text_speed_name
            });

        } else {//edit data in current session
            for (var i = 0; i < aaData.length; i++) {
                if (aaData[i].id == id) {
                    aaData[i].scrolling_text = scrolling_text;
                    aaData[i].duration_minute = duration;
                    aaData[i].speed_level_id = text_speed_id;
                    aaData[i].speed_level_name = text_speed_name;
                    break;
                }
            }
    ...
    function onEditTime(id) {
        ...
        if (id > 0) {
            ...
            $.each(editjsonobject, function (key, val) {
                ...
                $('#txtScollingText').val(val.scrolling_text);
                $('#txtAutoStopDuration').val(val.duration_minute);
                populateSelectTextSpeed(val.speed_level_id);
               // $('#cbTextSpeed').val(val.id);
            })
    ...
    function onValidTime() {
        ...
        var scrolling_text = $('#txtScollingText').val();
        var duration = $('#txtAutoStopDuration').val();
        var text_speed = $('#cbTextSpeed').val();
