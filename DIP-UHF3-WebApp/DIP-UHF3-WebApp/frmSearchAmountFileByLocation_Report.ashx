<%@ WebHandler Language="VB" Class="frmSearchAmountFileByLocation_Report" %>
...
Public Class frmSearchAmountFileByLocation_Report : Implements IHttpHandler
    
    Public Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        Dim url As String = context.Request.RawUrl
        Dim strTemp As String() = url.Split("&")
        Dim floor_id, patent_type_id, isshow As String
        ...
        Using ep As New ExcelPackage
            Dim ws As ExcelWorksheet = ep.Workbook.Worksheets.Add("AmountFileLocation")
            ws.Cells("A" & countCriteria & "").Value = "ลำดับ"
            ws.Cells("B" & countCriteria & "").Value = "พื้นที่"
            ...
            dt = webs.GetSearchAmountFileByLocationToDatatable(floor_id, patent_type_id, isshow)
            For i As Integer = 0 To dt.Rows.Count - 1
                countCriteria += 1
                ws.Cells("A" & countCriteria & "").Value = dt.Rows(i)("no").ToString
                ws.Cells("B" & countCriteria & "").Value = dt.Rows(i)("floor_name").ToString
            ...
            context.Response.AddHeader("content-disposition", "attachment;  filename=" & "EportFileQtyByFloor_" & DateTime.Now.ToString("yyyyMMddhhmmss") & ".xlsx")
