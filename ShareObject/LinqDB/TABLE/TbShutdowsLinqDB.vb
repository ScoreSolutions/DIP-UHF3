...
Namespace TABLE
    'Represents a transaction for TB_SHUTDOWS table LinqDB.
    '[Create by  on January, 15 2015]
    Public Class TbShutdowsLinqDB
        Public sub TbShutdowsLinqDB()
       ...
       'Define Public Method 
        ...
        '/// <param name=trans>The System.Data.SQLClient.SQLTransaction used by this System.Data.SQLClient.SQLCommand.</param>
        '/// <returns>true if data is retrieved successfully; otherwise, false.</returns>
        Public Function GetDataByPK(cID As Long, trans As SQLTransaction) As TbShutdowsLinqDB
        ...
        Private Function SetParameterData() As SqlParameter()
            ...
            cmbParam(2) = New SqlParameter("@_OPEN_DATETIME", SqlDbType.DateTime)
            If _OPEN_DATETIME.Value.Year > 1 Then 
                cmbParam(2).Value = _OPEN_DATETIME.Value
            Else
                cmbParam(2).Value = DBNull.value
            End If

            cmbParam(3) = New SqlParameter("@_CLOSE_DATETIME", SqlDbType.DateTime)
            If _CLOSE_DATETIME.Value.Year > 1 Then 
                cmbParam(3).Value = _CLOSE_DATETIME.Value
            Else
                cmbParam(3).Value = DBNull.value
            End If

            cmbParam(4) = New SqlParameter("@_S_STATUS", SqlDbType.Char)
            If _S_STATUS.Value <> "" Then 
                cmbParam(4).Value = _S_STATUS.Value
            Else
        ...
        '/// <param name=trans>The System.Data.SQLClient.SQLTransaction used by this System.Data.SQLClient.SQLCommand.</param>
        '/// <returns>true if data is retrieved successfully; otherwise, false.</returns>
        Private Function doGetData(whText As String, trans As SQLTransaction) As TbShutdowsLinqDB
            ClearData()
            _haveData  = False
            If whText.Trim() <> "" Then
                Dim tmpWhere As String = " WHERE " & whText
        ...
        Private ReadOnly Property SqlInsert() As String 
            Get
                ...
                Sql += " OUTPUT INSERTED.ID, INSERTED.COM_NAME, INSERTED.OPEN_DATETIME, INSERTED.CLOSE_DATETIME, INSERTED.S_STATUS, INSERTED.CREATEBY, INSERTED.CREATEON, INSERTED.UPDATEBY, INSERTED.UPDATEON
                Sql += " VALUES("
                sql += "@_ID" & ", "
                sql += "@_COM_NAME" & ", "
                sql += "@_OPEN_DATETIME" & ", "
        ...
        Private ReadOnly Property SqlSelect() As String
            Get
                Dim Sql As String = "SELECT ID, COM_NAME, OPEN_DATETIME, CLOSE_DATETIME, S_STATUS, CREATEBY, CREATEON, UPDATEBY, UPDATEON FROM " & tableName
                Return Sql
            End Get
        End Property
