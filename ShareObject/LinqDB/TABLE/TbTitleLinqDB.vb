...
Namespace TABLE
    'Represents a transaction for TB_TITLE table LinqDB.
    '[Create by  on January, 15 2015]
    Public Class TbTitleLinqDB
        Public sub TbTitleLinqDB()
       ...
       'Define Public Method 
        ...
        '/// <param name=trans>The System.Data.SQLClient.SQLTransaction used by this System.Data.SQLClient.SQLCommand.</param>
        '/// <returns>true if data is retrieved successfully; otherwise, false.</returns>
        Public Function GetDataByPK(cID As Long, trans As SQLTransaction) As TbTitleLinqDB
        ...
        Private Function SetParameterData() As SqlParameter()
            ...
            cmbParam(1) = New SqlParameter("@_TITLE_CODE", SqlDbType.VarChar)
            If _TITLE_CODE.Trim <> "" Then 
                cmbParam(1).Value = _TITLE_CODE
            Else
                cmbParam(1).Value = DBNull.value
            End If

            cmbParam(2) = New SqlParameter("@_TITLE_NAME", SqlDbType.VarChar)
            If _TITLE_NAME.Trim <> "" Then 
                cmbParam(2).Value = _TITLE_NAME
            Else
        ...
        '/// <param name=trans>The System.Data.SQLClient.SQLTransaction used by this System.Data.SQLClient.SQLCommand.</param>
        '/// <returns>true if data is retrieved successfully; otherwise, false.</returns>
        Private Function doGetData(whText As String, trans As SQLTransaction) As TbTitleLinqDB
            ClearData()
            _haveData  = False
            If whText.Trim() <> "" Then
                Dim tmpWhere As String = " WHERE " & whText
        ...
        Private ReadOnly Property SqlInsert() As String 
            Get
                ...
                Sql += " OUTPUT INSERTED.ID, INSERTED.TITLE_CODE, INSERTED.TITLE_NAME, INSERTED.CREATEBY, INSERTED.CREATEON, INSERTED.UPDATEBY, INSERTED.UPDATEON
                Sql += " VALUES("
                sql += "@_ID" & ", "
                sql += "@_TITLE_CODE" & ", "
                sql += "@_TITLE_NAME" & ", "
        ...
        Private ReadOnly Property SqlSelect() As String
            Get
                Dim Sql As String = "SELECT ID, TITLE_CODE, TITLE_NAME, CREATEBY, CREATEON, UPDATEBY, UPDATEON FROM " & tableName
                Return Sql
            End Get
        End Property
